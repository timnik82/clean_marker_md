...
def main() -> int:
        ...
        for path in sorted(in_dir.rglob(f"*{args.ext}")):
            ...
            if args.stats:
                ...
                if removed_count > 0 or anchors_count > 0:
                    print(
                        f"{path.name}: {removed_count} images, {anchors_count} page anchors"
                    )
    ...
    if args.stats:
        ...
        anchors_count = count_page_anchors(content)
        print(
            f"Found {removed_count} image placeholders and {anchors_count} page anchors to remove"
        )
    ...
    if args.dry_run:
        ...
        if args.stats:
            print(
                f"Would remove {removed_count} image placeholders and {anchors_count} page anchors"
            )
        return 0
    ...
    if args.stats:
        print(
            f"Removed {removed_count} image placeholders and {anchors_count} page anchors"
        )
    return 0
